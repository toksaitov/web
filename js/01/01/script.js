...
function calculateRadius() {
    if (isFieldEmpty('radius')) {
        hideEverything();
        return;
    }

    const radius = getNumericField('radius');
    ...
    updatePlaceholder('radius-placeholder', radius.toFixed(2));
...
function setupEventHandlers() {
    const form = getForm();
    const eventHandler = buildEventHandler(calculateRadius);
